# Doordash AI Data Engineering — End-to-End Project

A complete batch data pipeline that takes Doordash-style food delivery data from raw CSVs all the way to AI-powered analytics:

**Doordash/Food Delivery Dataset → Amazon S3 → Snowflake → dbt → Airflow → AI (OpenAI)**
...
![Architecture](C:\Users\madin\Downloads\architecture)
...
| **Bronze** | Snowflake `DOORDASH.RAW` | `COPY INTO` from S3 via a keyless storage integration |
| **Silver** | Snowflake `DOORDASH.STAGING` | dbt staging views — clean, type, rename every source |
| **Gold** | Snowflake `DOORDASH.MARTS` | Dimensions, **incremental** facts (MERGE), business marts + an SCD2 snapshot |
| **AI** | Snowflake `DOORDASH.AI` | LLM-enriched reviews (sentiment/topic), RAG chat, text-to-SQL |
...
│   └── dags/doordash_batch.py  #   the pipeline DAG (4 tasks)
├── DOORDASH-PIPELINE/                   # dbt project
...
├── ai/                       # AI layer
│   ├── enrich_reviews.py     #   LLM enrichment → DOORDASH.AI.REVIEW_ENRICHED
...
│   ├── 01_setup.sql          #   warehouse DOORDASH_WH, database DOORDASH, schemas, role
...
Table DDL ([`snowflake/04_raw_tables.sql`](snowflake/04_raw_tables.sql)) matches each CSV's column order, then [`snowflake/05_copy_into.sql`](snowflake/05_copy_into.sql) pulls each file from the stage into `DOORDASH.RAW` tables: 10M orders, ~23M order items, 300K reviews.
...
1. **LLM enrichment** (`ai/enrich_reviews.py`) — *LLM as a transformation step.* Reads review text, asks `gpt-4o-mini` for structured JSON (sentiment + topic), writes it back to `DOORDASH.AI.REVIEW_ENRICHED` — which dbt then models into `mart_review_insights` like any other table. Idempotent and sample-capped (`SAMPLE_N`) so you never pay twice for the same review.
...
```bash
# Snowflake objects (warehouse DOORDASH_WH, database DOORDASH, schemas RAW/STAGING/MARTS/SNAPSHOTS/AI, role DBT_ROLE)
...
# http://localhost:8080 → un-pause DOORDASH_batch → Trigger
